...
public class NetManager
{
    ...
    public bool Send(int msgno,object obj)
    {
		if (!this.isConnected) {
			Debug.Log ("网络连接己断开");
			return false;		
    ...
    private void RecvFunc()
    {
        ...
        while (mThreadRunning && this.isConnected)
        {
            int len = 0;
            try
            {
                ...
                Debug.Log("mStream.Read error: " + e.Message);
            }
            Debug.Log("recv len: " + len);
            if (len >= 6)
            {
